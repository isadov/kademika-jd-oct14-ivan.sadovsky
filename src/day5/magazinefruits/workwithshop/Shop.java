...
public class Shop {
    ...
    public void makePurchase(String customerName, Denomination fruitName, int number) {
        int count = 0;
        System.out.println("We have " + storages.getFruitBalance(fruitName) + " " + fruitName + " in storage.");
        if (number <= storages.getFruitBalance(fruitName)) {
            ...
            System.out.println("There are only " + storages.getFruitBalance(fruitName) + " " + fruitName + " in storage");

        storages.insertPurchase(purchase);
        customer = new Customer();
        customer.setName(customerName);
        storages.insertCustomer(customer);
        System.out.println(customerName + " bought " + count + " " + fruitName + "(s).");
        System.out.println("Balance changes. " + fruitName + " left in storage : " + storages.getFruitBalance(fruitName));
